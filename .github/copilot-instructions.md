...
- Creating shortcut/alias variables for attributes or functions that are only used inline/once. Never write `x = obj.attr` then use `x` only once — write `obj.attr` directly at its usage site. No exceptions.
