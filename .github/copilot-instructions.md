...
- Creating shortcut/alias variables for attributes or functions. Never write `x = obj.attr` and then use `x` — always write `obj.attr` directly at every usage site. No exceptions.
