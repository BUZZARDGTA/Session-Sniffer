...
class SessionSummaryWindow(ToggleAlwaysOnTopMixin):
    ...
    def __init__(self, *, always_on_top: bool = True) -> None:
        ...
        bandwidth_form.addRow('Total Exchanged:', self._label_bandwidth)
        bandwidth_form.addRow('Downloaded:', self._label_download)
        bandwidth_form.addRow('Uploaded:', self._label_upload)
        layout.addWidget(bandwidth_group)

        # Rates group
        rates_group = QGroupBox('Rates')
        rates_form = QFormLayout(rates_group)
        self._label_pps = QLabel('0 PPS')
        self._label_bps = QLabel('0 B/s')
        self._label_peak_pps = QLabel('0 PPS')
        self._label_peak_bps = QLabel('0 B/s')
        rates_form.addRow('Current PPS:', self._label_pps)
        rates_form.addRow('Current BPS:', self._label_bps)
        rates_form.addRow('Peak PPS:', self._label_peak_pps)
        rates_form.addRow('Peak BPS:', self._label_peak_bps)
        ...
        misc_form.addRow('Window Uptime:', self._label_uptime)
        misc_form.addRow('Capture Restarts:', self._label_restarts)
    ...
    def refresh(self) -> None:
        ...
        self._label_connected.setText(str(len(connected)))
        self._label_total.setText(str(len(all_players)))
        self._label_bandwidth.setText(PlayerBandwidth.format_bytes(total_bandwidth))
        self._label_download.setText(PlayerBandwidth.format_bytes(total_download))
        self._label_upload.setText(PlayerBandwidth.format_bytes(total_upload))
        self._label_pps.setText(f'{CaptureStats.global_pps_rate} PPS')
        self._label_bps.setText(f'{PlayerBandwidth.format_bytes(CaptureStats.global_bps_rate)}/s')
        self._label_peak_pps.setText(f'{self._peak_pps} PPS')
        self._label_peak_bps.setText(f'{PlayerBandwidth.format_bytes(self._peak_bps)}/s')
        self._label_uptime.setText(uptime)
