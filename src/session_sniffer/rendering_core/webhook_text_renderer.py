...
def format_player_column_value(player: Player, column_name: str, now: datetime) -> str:
    ...
    formatters: dict[str, Callable[[], str]] = {
        ...
        'Region': lambda: str(player.iplookup.ipapi.region),
        'R. Code': lambda: str(player.iplookup.ipapi.region_code),
        'City': lambda: str(player.iplookup.geolite2.city),
        'District': lambda: str(player.iplookup.ipapi.district),
        'ZIP Code': lambda: str(player.iplookup.ipapi.zip_code),
        'Lat': lambda: str(player.iplookup.ipapi.lat),
        'Lon': lambda: str(player.iplookup.ipapi.lon),
        'Time Zone': lambda: str(player.iplookup.ipapi.time_zone),
        'Offset': lambda: str(player.iplookup.ipapi.offset),
        'Currency': lambda: str(player.iplookup.ipapi.currency),
        'Organization': lambda: str(player.iplookup.ipapi.org),
        'ISP': lambda: str(player.iplookup.ipapi.isp),
        'ASN / ISP': lambda: player.iplookup.geolite2.asn,
        'AS': lambda: str(player.iplookup.ipapi.asn),
        'ASN': lambda: str(player.iplookup.ipapi.as_name),
